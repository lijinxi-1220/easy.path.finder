...
请求体：`message_content`、可选 `chat_scene`、可选 `chat_id`（续写会话）
响应字段：`reply_content|timestamp|chat_id`
示例：
...
请求参数：`resume_id`
响应字段：`guide_question|missing_field`
示例：
...
请求参数：分页 `page|page_size`
响应字段：`chats` 列表项含 `chat_id|message_content|reply_content|timestamp` 与 `meta`
示例：
...
请求参数：`chat_id`，分页 `page|page_size`
响应字段：`messages` 列表项含 `message_id|role|message_content|reply_content|timestamp` 与 `meta`
示例：
